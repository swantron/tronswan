...
describe('RecipeCard Component', () => {
  ...
  test('renders recipe card with all elements', () => {
    ...
    expect(
      screen.getByText('This is a test recipe excerpt')
    ).toBeInTheDocument();
  ...
  test('does not render image when featuredImage is null', () => {
    ...
    expect(
      screen.queryByAltText('Recipe Without Image')
  ...
  test('renders category elements with correct structure', () => {
    ...
    expect(breakfastCategory.parentElement).toHaveClass(
  ...
  test('handles recipe with very long title', () => {
    const longTitleRecipe = {
      ...mockRecipe,
      title:
        'This is a very long recipe title that might exceed normal display limits and should be handled gracefully by the component',
    };
  ...
  test('handles recipe with HTML in excerpt', () => {
    const htmlExcerptRecipe = {
      ...mockRecipe,
      excerpt:
        '<h2>HTML Title</h2><p>Paragraph with <strong>bold</strong> text</p>',
    };
