...
// Mock the CSS import
vi.mock('../styles/SwantronDetail.css', () => ({}));
...
describe('SwantronDetail Component', () => {
  const mockPost = {
    ...
    excerpt:
      '<p>This is a test post excerpt with <strong>HTML</strong> content.</p>',
    content:
      '<h2>Post Content</h2><p>This is the main post content with <em>emphasis</em>.</p>',
  ...
  test('renders post details when data is loaded successfully', async () => {
    ...
    expect(
      screen.getByText(/This is the main post content with/)
    ).toBeInTheDocument();
  ...
  test('renders SEO component with correct props', async () => {
    ...
    expect(seoComponent).toHaveAttribute(
      'data-title',
      'Test Post Title - Swantron | Tron Swan'
    );
    expect(seoComponent).toHaveAttribute(
      'data-description',
      'This is a test post excerpt with HTML content.'
    );
    expect(seoComponent).toHaveAttribute(
      'data-keywords',
      'swantron, Joseph Swanson, Test Post Title, blog post'
    );
  ...
  test('renders external link with correct attributes', async () => {
    ...
    const externalLink = screen.getByText('goto swantron.com ↗');
  ...
  test('renders not found state when post is null', async () => {
    ...
    await waitFor(() => {
      expect(
        screen.getByTestId('swantron-detail-not-found')
      ).toBeInTheDocument();
    });
  ...
  test('handles post with special characters in title', async () => {
    ...
    expect(
      screen.getByText('Post with special chars: & < > " \' é ñ 🚀')
    ).toBeInTheDocument();
  });
  ...
  test('handles post with special characters in content', async () => {
    ...
    expect(
      screen.getByText('Content with & < > " \' é ñ 🚀')
    ).toBeInTheDocument();
  });
