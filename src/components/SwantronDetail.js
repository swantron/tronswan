...
const SwantronDetail = () => {
  ...
  useEffect(() => {
    const fetchPost = async () => {
      try {
        ...
        setError('Failed to load post from swantron.com. Please try again later.');
  ...
  if (loading) {
    return (
      <div className="swantron-detail-loading" data-testid="swantron-detail-loading">
  ...
  if (error) {
    return (
      <div className="swantron-detail-error" data-testid="swantron-detail-error">
        {error}
      </div>
    );
  }
  ...
  if (!post) {
    return (
      <div className="swantron-detail-not-found" data-testid="swantron-detail-not-found">
        Post not found.
      </div>
    );
  }
        ...
        <div className="swantron-detail-content">
          <header className="swantron-detail-header">
            ...
            <div className="swantron-detail-meta">
              <time className="swantron-detail-date">{formatDate(post.date)}</time>
              {post.categories.length > 0 && (
                <div className="swantron-detail-categories">
                  {post.categories.map(category => (
                    <span key={category.id} className="swantron-detail-category">
                      {category.name}
                    </span>
                  ))}
                </div>
