...
test.describe('Weather Page E2E Tests', () => {
  test('Weather page displays temperature in correct format', async ({ page }) => {
  ...
  test('Weather page refreshes data on reload', async ({ page }) => {
    ...
    expect(firstTemperature).toMatch(testData.expectedContent.weather.temperaturePattern);
    expect(secondTemperature).toMatch(testData.expectedContent.weather.temperaturePattern);
  });
});
